
class Solution:
    tries = 0
    unhappy_nums = (4,16,37,58,89,145,42,20)
    def isHappy(self, n: int) -> bool:
        x = n
        squaresum = 0
        while (x != 0):
            squaresum += (x % 10)**2

            x = int(x/10)
        if squaresum == 1:
            self.tries = 0
            return True

        if squaresum in set(self.unhappy_nums) or self.tries > 20:
            self.tries = 0
            return False
        self.tries += 1
        print(f"trying {self.tries} times, now square sum is {squaresum}")

        return self.isHappy(squaresum)
        

s = Solution()

def test_is_happy_sample():
    assert s.isHappy(19) == True

def test_is_happy_blind_try():
    assert s.isHappy(888) == True

def test_is_happy_blind_try2():
    assert s.isHappy(88888) == True

def test_is_happy_blind_try3():
    assert s.isHappy(12345) == False

def test_is_happy_blind_find():
    happy = []
    for x in range(1, 2048):
        if s.isHappy(x):
            happy.append(x)
    print(happy)

def test_is_happay_all():
    exp = [1, 7, 10, 13, 19, 23, 28, 31, 32, 44, 49, 68, 70, 79, 82, 86, 91, 94, 97, 100, 103, 109, 129, 130, 133, 139, 167, 176, 188, 190, 192, 193, 203, 208, 219, 226, 230, 236, 239, 262, 263, 280, 291, 293, 301, 302, 310, 313, 319, 320, 326, 329, 331, 338, 356, 362, 365, 367, 368, 376, 379, 383, 386, 391, 392, 397, 404, 409, 440, 446, 464, 469, 478, 487, 490, 496, 536, 556, 563, 565, 566, 608, 617, 622, 623, 632, 635, 637, 638, 644, 649, 653, 655, 656, 665, 671, 673, 680, 683, 694, 700, 709, 716, 736, 739, 748, 761, 763, 784, 790, 793, 802, 806, 818, 820, 833, 836, 847, 860, 863, 874, 881, 888, 899, 901, 904, 907, 910, 912, 913, 921, 923, 931, 932, 937, 940, 946, 964, 970, 973, 989, 998, 1000, 1003, 1009, 1029, 1030, 1033, 1039, 1067, 1076, 1088, 1090, 1092, 1093, 1112, 1114, 1115, 1121, 1122, 1125, 1128, 1141, 1148, 1151, 1152, 1158, 1177, 1182, 1184, 1185, 1188, 1209, 1211, 1212, 1215, 1218, 1221, 1222, 1233, 1247, 1251, 1257, 1258, 1274, 1275, 1277, 1281, 1285, 1288, 1290, 1299, 1300, 1303, 1309, 1323, 1330, 1332, 1333, 1335, 1337, 1339, 1353, 1366, 1373, 1390, 1393, 1411, 1418, 1427, 1444, 1447, 1448, 1457, 1472, 1474, 1475, 1478, 1481, 1484, 1487, 1511, 1512, 1518, 1521, 1527, 1528, 1533, 1547, 1557, 1572, 1574, 1575, 1578, 1581, 1582, 1587, 1599, 1607, 1636, 1663, 1666, 1670, 1679, 1697, 1706, 1717, 1724, 1725, 1727, 1733, 1742, 1744, 1745, 1748, 1752, 1754, 1755, 1758, 1760, 1769, 1771, 1772, 1784, 1785, 1796, 1808, 1812, 1814, 1815, 1818, 1821, 1825, 1828, 1841, 1844, 1847, 1851, 1852, 1857, 1874, 1875, 1880, 1881, 1882, 1888, 1900, 1902, 1903, 1920, 1929, 1930, 1933, 1959, 1967, 1976, 1992, 1995, 2003, 2008, 2019, 2026, 2030, 2036, 2039]
    happy = []
    for x in range(1, 2048):
        if s.isHappy(x):
            happy.append(x)
    assert happy == exp
